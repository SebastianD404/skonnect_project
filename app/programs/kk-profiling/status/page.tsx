...
import { AlertCircle, ArrowLeft, ArrowRight, CheckCircle2, Clock3, ExternalLink, FileText, History, ShieldCheck, X } from "lucide-react";
...
export default function KKProfilingStatusPage() {
  ...
  const normalizedProfileStatus = profile?.status.toLowerCase() ?? "";
  const isReturnedStatus = normalizedProfileStatus.includes("returned");
  const isHistoricalReviewerNotes = /resubmitted|pending/.test(normalizedProfileStatus);
  const hasReviewerNotes = Boolean(profile?.reviewNotes?.trim() && (isReturnedStatus || isHistoricalReviewerNotes));
          ...
          <div className="mb-8 space-y-4">
            ...
            <div>
              ...
              <p className="mt-1 text-sm text-slate-500">
                Monitor your registration review status, verify submitted documents, and track your membership progress.
              </p>
            </div>
          </div>
          ...
          {loading ? (
            ...
            <div className="space-y-6">
              ...
              <div className="bg-white border border-slate-200/80 rounded-2xl p-6 shadow-sm flex flex-col justify-between h-full">
                <div>
                  <h3 className="text-lg font-semibold tracking-tight text-slate-900">Verification Documents</h3>
                  <p className="mt-1 text-sm leading-relaxed text-slate-500">Review the official identification and residency documents submitted for this application.</p>
                  <div className="mt-5 space-y-4">
                    {[
                      ["Front of ID", profile.idFrontFileUrl, "Primary Credential", "JPG document"],
                      ["Back of ID", profile.idBackFileUrl, "Secondary Credential", "JPG document"],
                      ["Certificate of Residency", profile.idSingleFileUrl, "Proof of Address", "PDF document"],
                    ].map(([title, url, subtitle, metadata]) => url ? (
                      <div key={title} className="group flex items-center justify-between rounded-2xl border border-slate-200/80 bg-white p-4 shadow-sm transition-colors hover:border-slate-300">
                        <div className="flex min-w-0 items-center gap-3">
                          <img src={url} alt={`${title} preview`} className="h-12 w-12 shrink-0 rounded-xl border border-slate-200 object-cover" />
                        ...
                        <div className="flex shrink-0 items-center gap-4">
                          <span className="hidden text-xs font-medium text-slate-400 sm:inline">{metadata}</span>
                          <a
                            href={url}
                            target="_blank"
                    ...
                    {!profile.idFrontFileUrl && !profile.idBackFileUrl && !profile.idSingleFileUrl ? (
                      <p className="rounded-xl border border-slate-100 bg-slate-50 p-4 text-sm text-slate-600">No uploaded ID documents are stored for this registration.</p>
                    ) : null}
                  </div>
                </div>
                ...
                {isApprovedStatus === true ? (
                  <div className="mt-8 border-t border-slate-100 pt-6">
                    <h4 className="mb-4 text-xs font-bold uppercase tracking-wider text-slate-400">What Happens Next?</h4>
              ...
              {isApprovedStatus === true ? (
                    ...
                    <aside className="bg-white border border-slate-200/80 rounded-2xl p-6 shadow-sm flex flex-col justify-between h-full lg:sticky lg:top-24">
                    ...
                    <>
                      {hasReviewerNotes ? (
                        <section>
                          <div className="flex items-start justify-between gap-4">
                            <div>
                              ...
                              <p className="mt-2 text-sm text-slate-600">
                                {isHistoricalReviewerNotes
                                  ? "Previous feedback from staff (addressed in your resubmission)."
                                  : "Feedback from Barangay Pico staff for your KK Profiling submission."}
                              </p>
                            </div>
                            {isReturnedStatus ? (
                              <span className="inline-flex items-center rounded-full bg-amber-100 px-3 py-1 text-xs font-bold tracking-wide text-amber-700">
                                ACTION REQUIRED
                              </span>
                            ) : (
                              <span className="inline-flex items-center rounded-full bg-slate-100 px-3 py-1 text-xs font-bold tracking-wide text-slate-500">
                                PREVIOUS FEEDBACK
                              </span>
                            )}
                          </div>
                          ...
                          <div
                            className={`mt-3 mb-6 flex items-start gap-3 rounded-lg border p-4 text-sm ${
                              isReturnedStatus
                          ...
                          >
                            {isReturnedStatus ? (
                              ...
                              <History className="mt-0.5 h-5 w-5 shrink-0 text-slate-400" aria-hidden="true" />
                            )}
                            <p className="font-medium leading-7">{profile.reviewNotes}</p>
                          </div>
                        </section>
                      ) : null}
